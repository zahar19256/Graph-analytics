...
int64_t HashPair(int32_t i, int32_t j) { return i * kMaxBucketCount + j; }
...
void Convertor::ToBinaryConvertation(const std::string &input_file,
...
void Convertor::WriteMeta(MmapWriter &writer, size_t meta_offset) const {
  ...
  GraphMetaHeader header{static_cast<uint64_t>(meta_.GetEdgeCount()),
  ...
  for (const std::pair<const int64_t, batch_info> &item : meta_.GetBatches()) {
    ...
    GraphMetaBatchRecord record{item.first, static_cast<uint64_t>(info.offset),
