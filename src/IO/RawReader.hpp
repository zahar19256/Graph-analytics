...
public:
  ...
  template <typename T> bool Read(T &value) {
    return ReadRawBytes(reinterpret_cast<char *>(&value), sizeof(T)) ==
           sizeof(T);
  }
  ...
  size_t ReadVector(std::vector<T> &values, size_t count) {
    values.resize(count);
    if (!values.empty()) {
      size_t read_bytes = ReadRawBytes(reinterpret_cast<char *>(values.data()),
  ...
  template <typename T> size_t ReadVector(std::vector<T> &values) {
  ...
  bool Empty() { return in_.peek() == std::ifstream::traits_type::eof(); }
