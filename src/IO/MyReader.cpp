...
void MyReader::ReadBatchPart(size_t index, size_t edge_offset, size_t count, std::vector<Edge>& storage) {
    ...
    ReadRawBytes(byte_offset, reinterpret_cast<char*>(storage.data()), byte_size);
}
...
void MyReader::ReadEdges(std::vector<Edge>& storage) {
    ...
    ReadRawBytes(0, reinterpret_cast<char*>(storage.data()), count * sizeof(Edge));
}
...
void MyReader::ReadMeta() {
    ...
    ReadRawBytes(header_offset, reinterpret_cast<char*>(&header), sizeof(header));
    if (header.from_batch_size != kFromBatchSize || header.to_batch_size != kToBatchSize) {
        throw std::runtime_error("Graph meta batch size does not match current binary!");
    }
    if (header.edge_count > std::numeric_limits<size_t>::max() / sizeof(Edge)) {
        throw std::runtime_error("Graph edge section size overflow!");
    }
    if (header.batch_count > std::numeric_limits<size_t>::max() / sizeof(GraphMetaBatchRecord)) {
    ...
    size_t batch_bytes = static_cast<size_t>(header.batch_count) * sizeof(GraphMetaBatchRecord);
    if (edge_bytes > size_ ||
        batch_bytes > size_ - edge_bytes ||
    ...
    for (uint64_t i = 0; i < header.batch_count; ++i) {
        ...
        ReadRawBytes(record_offset, reinterpret_cast<char*>(&record), sizeof(record));
        meta_.InsertInfo(
            record.index,
            {
                static_cast<size_t>(record.offset),
...
size_t MyReader::GetEdgeCount() const {
    return meta_.GetEdgeCount();
}

size_t MyReader::GetVertexCount() const {
    return meta_.GetVertexCount();
}

const MetaData& MyReader::GetMetaData() const {
    return meta_;
}
