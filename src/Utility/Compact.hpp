...
// у меня на последних этапах реализации было осознанно, что из-за пробелов в
// номерах вершин место с хранением встепеней и рангов вершин становится
// огромным буквально sizeof(VertexInfo) * max_node что сильно больше, чем
// реально нужно
...
делаем сорт вершин сначал по from и замену номеров у from, потом сорт по to и
аналогично замена номеров по to Такой алгоритм уплотняет рёбра и решает проблему
...
https://snap.stanford.edu/data/ego-Twitter.html из-за чего каждая итерация
алгоритма длилась от 70 до 100 секунд, так как было много переходов по почти
пустому файлу вершин
*/
...
class HalfToCompare {
public:
  bool operator()(const HalfMappedEdge &left,
                  const HalfMappedEdge &right) const {
...
static void SortEdgesByFrom(const std::string &file) {
  ...
  SortFile<Edge, EdgeFromCompare> sort_file(file + ".edges_from", 0,
                                            EdgeFromCompare(), false);
...
static void SortHalfByTo(const std::string &file) {
  ...
  SortFile<HalfMappedEdge, HalfToCompare> sort_file(file + ".half_to", 0,
                                                    HalfToCompare(), false);
