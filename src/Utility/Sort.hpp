...
public:
  ...
  bool HasValue() const { return position_ < size_; }

  const T &Value() const { return buffer_[position_]; }
...
public:
  ...
  bool operator()(const SortHeapNode<T> &left,
                  const SortHeapNode<T> &right) const {
...
MergeBlocks(const std::string &input_file, const std::vector<size_t> &offsets,
  ...
  for (size_t i = 0; i < offsets.size(); ++i) {
    readers.push_back(std::unique_ptr<SortBlockReader<T>>(
        new SortBlockReader<T>(input_file, start, offsets[i])));
    start += offsets[i];
  }
  std::priority_queue<SortHeapNode<T>, std::vector<SortHeapNode<T>>,
                      SortHeapCompare<T, Compare>>
      heap((SortHeapCompare<T, Compare>(compare)));
...
public:
  SortFile(const std::string &base_path, size_t index, Compare compare,
      ...
      : base_path_(base_path),
        path_(base_path + ".sort_" + std::to_string(index)), index_(index),
        compare_(compare), unique_(unique) {
    ClearFile(path_);
  }
  ...
  void Merge() {
    ...
    if (!next_) {
      next_.reset(
          new SortFile<T, Compare>(base_path_, index_ + 1, compare_, unique_));
    }
    std::string merged_file =
        base_path_ + ".sort_merge_" + std::to_string(index_);
    size_t merged_count = MergeBlocks<T, Compare>(path_, offsets_, merged_file,
                                                  compare_, unique_);
...
class IntCompare {
public:
  bool operator()(int32_t left, int32_t right) const { return left < right; }
};
