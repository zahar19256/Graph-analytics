...
struct VertexFile {
  ...
  void ReadChunk(size_t first_vertex, size_t count,
                 std::vector<VertexInfo> &storage) {
    ...
    ssize_t read_bytes =
        pread(fd, storage.data(), bytes, first_vertex * sizeof(VertexInfo));
  ...
  void WriteChunk(size_t first_vertex, const std::vector<VertexInfo> &storage) {
    ...
    ssize_t written_bytes =
        pwrite(fd, storage.data(), bytes, first_vertex * sizeof(VertexInfo));
...
void Executor::Compute() {
  ...
  const std::unordered_map<int64_t, batch_info> &meta_batches =
      graph_reader.GetMetaData().GetBatches();
  for (std::unordered_map<int64_t, batch_info>::const_iterator it =
           meta_batches.begin();
       it != meta_batches.end(); ++it) {
...
#pragma omp parallel for num_threads(kOpenMpThreadCount) schedule(dynamic)     \
    reduction(+ : ground_next_rank) reduction(max : max_rank_diff)
    for (long long to_batch_id = 0;
         to_batch_id < static_cast<long long>(to_batch_count); ++to_batch_id) {
      ...
      for (int64_t index : batches_by_to[to_batch]) {
        ...
        size_t from_count =
            GetChunkSize(from_batch, kFromBatchSize, vertex_count);
        size_t batch_size = graph_reader.GetMetaData().GetSize(index);

        vertex_file.ReadChunk(from_start, from_count, from_vertices);
        for (size_t edge_offset = 0; edge_offset < batch_size;
             edge_offset += kEdgeReadBatchSize) {
          size_t edge_count =
              std::min(kEdgeReadBatchSize, batch_size - edge_offset);
          graph_reader.ReadBatchPart(static_cast<size_t>(index), edge_offset,
                                     edge_count, edges);
      ...
      for (size_t i = 0; i < to_count; ++i) {
        double diff = std::abs(static_cast<double>(to_vertices[i].rank) -
                               static_cast<double>(to_vertices[i].next_rank));
...
#pragma omp parallel for num_threads(kOpenMpThreadCount) schedule(dynamic)
    ...
    std::chrono::steady_clock::time_point iteration_finish =
        std::chrono::steady_clock::now();
    double seconds =
        std::chrono::duration<double>(iteration_finish - iteration_start)
            .count();
    std::cerr << "LeaderRank iteration " << gen << " time: " << seconds
              << " sec" << std::endl;
...
void Executor::MakeResult(double ground_rank) {
  ...
  for (size_t to_batch = 0; to_batch < to_batch_count; ++to_batch) {
    ...
    for (size_t i = 0; i < to_count; ++i) {
      fprintf(out, "%d,%.17g\n", vertices[i].old_id,
              static_cast<double>(vertices[i].rank) + ground_add);
    }
  }
