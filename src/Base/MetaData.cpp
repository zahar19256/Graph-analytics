...
void MetaData::FillBatch(int64_t index, size_t size) {
  if (!batch_offset_.count(index)) {
    throw std::runtime_error(
        "No such index in meta data batch_info container: " +
        std::to_string(index));
  }
  if (batch_offset_[index].used_size + size > batch_offset_[index].size) {
    throw std::runtime_error("No space left in batch: " +
                             std::to_string(index));
  }
...
size_t MetaData::GetOffset(int64_t index) const {
  std::unordered_map<int64_t, batch_info>::const_iterator it =
      batch_offset_.find(index);
...
size_t MetaData::GetEdgeCount() const { return total_edge_count_; }

size_t MetaData::GetVertexCount() const { return vertex_count_; }
